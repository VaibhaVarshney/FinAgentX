...
def plot_price_chart(historical_df: pd.DataFrame, ticker: str):
    ...
    fig.add_trace(go.Scatter(
        ...
        name="MA200",
        line=dict(color=COLORS["blue"], width=1, dash="dash"),
    ), row=1, col=1)
    ...
    fig.update_layout(
        **LAYOUT_BASE,
        ...
        title=dict(text=f"⚠️ {ticker} — Risk Metrics",
                   font=dict(size=14, color=COLORS["green"])),
        yaxis=dict(
            showgrid=True, gridcolor=COLORS["border"], zeroline=True, zerolinecolor=COLORS["muted"]),
        showlegend=False,
    )
...
def plot_comparison_chart(analysis1: dict, analysis2: dict):
    """
    Side-by-side comparison: price charts + risk metrics.
    """
    t1 = analysis1["ticker"]
    t2 = analysis2["ticker"]
    ...
    fig_price.add_trace(go.Scatter(
        x=df1.index, y=df1["Normalized"],
        name=t1,
        line=dict(color=COLORS["green"], width=2),
    ))
    fig_price.add_trace(go.Scatter(
        x=df2.index, y=df2["Normalized"],
        name=t2,
    ...
    fig_price.update_layout(
        **LAYOUT_BASE,
        height=350,
        title=dict(text=f"📊 {t1} vs {t2} — Normalized Price (Base 100)", font=dict(
    ...
    v1 = [
        analysis1["risk_analysis"]["annual_volatility"],
        analysis1["risk_analysis"]["sharpe_ratio"],
        analysis1["risk_analysis"]["max_drawdown"],
    ]
    v2 = [
        analysis2["risk_analysis"]["annual_volatility"],
    ...
    fig_risk.add_trace(go.Bar(
        name=t1, x=metrics, y=v1,
        marker_color=COLORS["green"],
        text=[f"{v:.3f}" for v in v1],
        textposition="outside",
    ))
    fig_risk.add_trace(go.Bar(
        name=t2, x=metrics, y=v2,
        marker_color=COLORS["blue"],
        text=[f"{v:.3f}" for v in v2],
        textposition="outside",
    ))
